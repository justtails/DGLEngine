...
{                                                                              }
{  DRON's OpenGl Engine V 1.0a http://dronprogs.mirgames.ru                    }
{                                                                              }
{  Date: 25.04.2006                                                            }
{                                                                              }
...
interface
uses Windows, Tlhelp32, Classes, SysUtils, Graphics, OpenGl;

function  LoadDGLEngineDLL(FileName : string) : boolean;
procedure FreeDGLEngineDLL(EXE_FileName : string = '');
...
_glTexCoord2f : procedure (X,Y : GLFloat); stdcall;
...
begin
 ...
 if EXE_FileName<>'' then
   if KillTask(EXE_FileName)=0 then
   MessageBox(0, PChar('Couldn''t find process "'+EXE_FileName+'"!'),'DGLEngine header', MB_OK or MB_ICONERROR);
